...
def load_data():
    ...
    for i in patients:
        ...
        for j in scanners:
            ...
            for k in sequences2:
                INPUT_FOLDER4=os.path.join(INPUT_FOLDER3, k)
                patient = load_scan(INPUT_FOLDER4)
                first_patient_pixels, machine, datas = get_pixels_hu(patient)
                if (k==sequences2[0]) & (j==scanners[0]) & (i==patients[0]):
                    #totalpatients=np.array(first_patient_pixels)
                    #shortlabels=np.array(machine)
                    #print(totallabels)
                    if np.any(machine)==1:
                        machine1=np.array(first_patient_pixels)
                    else:
                        machine2=np.array(first_patient_pixels)
                else:
                    ...
                    if np.any(machine)==1:
                        if not len(machine1):
                            ...
                            machine1 = np.concatenate([np.array(machine1), np.array(first_patient_pixels)])
                    else:
                        if not len(machine2):
                            machine2=np.array(first_patient_pixels)
                        else:
                            machine2=np.concatenate([np.array(machine2), np.array(first_patient_pixels)])
...
def define_generator(image_shape=(256,256,1)):
    ...
    b = Conv2D(1024, (3,3), strides=(2,2), padding='same', kernel_initializer=init)(e7)
    #b = Conv2D(512, (3,3), strides=(2,2), padding='same')(e4)

    b = Conv2D(1024, (3,3), strides=(2,2), padding='same')(e7)
    b = Activation('relu')(b)
    # decoder model
...
def define_composite_model(g_model_1, d_model,d_model_C, g_model_2, image_shape):
    ...
    output_patri1=d_model_C(gen1_out)
    output_patri2=d_model_C(gen2_out)
    # define model graph
    model = Model([input_gen, input_id], [output_d, output_id, output_f, output_b,output_patri1,output_patri2])
    # define optimization algorithm configuration
    opt = Adam(lr=0.0002, beta_1=0.5)
    # compile model with weighting of least squares loss and L1 loss
    model.compile(loss=['mse', 'mae', 'mae', 'mae','mse','mse'], loss_weights=[1, 5, 10, 10, 10, 10], optimizer=opt)
    model_json = g_model_1.to_json()
    with open("modelg1.json", "w") as json_file:
        json_file.write(model_json)
    model_json2 = g_model_2.to_json()
    with open("modelg2.json", "w") as json_file:
...
def save_models(step, g_model_AtoB, g_model_BtoA):
    ...
    filename1 = 'results3/g_model_AtoB_%06d.h5' % (step+1)
    g_model_AtoB.save(filename1)
# save the second generator model
    filename2 = 'results3/g_model_BtoA_%06d.h5' % (step+1)
...
def summarize_performance(step, g_model, trainX, trainXB, name, nameB, n_samples=5):
    # select a sample of input images
    X_in, X_inB,_,_ = generate_real_samples(trainX,trainXB, n_samples, 0)
    # generate translated images
    X_out, _ = generate_fake_samples(g_model, X_in, 0)
    X_outB, _ = generate_fake_samples(g_model, X_inB, 0)
    ...
    filename1 = 'results3/%s_generated_plot_%06d.png' % (name, (step+1))
    pyplot.close()
    for i in range(n_samples):
        pyplot.subplot(2, n_samples, 1 + i)
        pyplot.axis('off')
    ...
    filename2 = 'results3/%s_generated_plot_%06d.png' % (nameB, (step+1))
...
def plot_history(dA1_hist, dA2_hist, dB1_hist,dB2_hist,g1_hist,g2_hist,dC1_hist,dC12_hist,dC2_hist,dC22_hist):
    # plot loss
    
    pyplot.plot(dA1_hist, label='d-real1')
    pyplot.plot(dB1_hist, label='d-real2')
    pyplot.plot(dA2_hist, label='d-fake1')
    pyplot.plot(dB2_hist, label='d-fake2')
    pyplot.plot(dC1_hist, label='dscan1-real')
    pyplot.plot(dC12_hist, label='dscan1-fake')
    pyplot.plot(dC2_hist, label='dscan2-real')
    pyplot.plot(dC22_hist, label='dscan2-fake')
    ...
    #pyplot.legend()
    # save plot to file
    filename3 = 'results3/plot_line_plot_loss.png'
...
def train(d_model_A, d_model_B,d_model_C, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, dataset):
    ...
    dA1_hist, dA2_hist, dB1_hist, dB2_hist, g1_hist, g2_hist,dC1_hist, dC12_hist,dC2_hist,dC22_hist= list(), list(), list(), list(), list(), list(),list(),list(),list(),list()
...
# generate a batch of fake samples
        ...
        g_loss2, disclossBA, identityBA, cycle1BA, cycle2BA, extradisc1BA, extradisc2BA = c_model_BtoA.train_on_batch([X_realB, X_realA], [y_realA, X_realA, X_realB, X_realA,y_fakeB,y_realA])
        # update discriminator for A -> [real/fake]
        dA_loss1 = d_model_A.train_on_batch(X_realA, y_realA)
        dA_loss2 = d_model_A.train_on_batch(X_fakeA, y_fakeA)
        # update generator A->B via adversarial and cycle loss
        g_loss1, disclossAB, identityAB, cycle1AB, cycle2AB, extradisc1AB, extradisc2AB = c_model_AtoB.train_on_batch([X_realA, X_realB], [y_realB, X_realB, X_realA, X_realB,y_realA,y_fakeB])
        # update discriminator for B -> [real/fake]
        dB_loss1 = d_model_B.train_on_batch(X_realB, y_realB)
        dB_loss2 = d_model_B.train_on_batch(X_fakeB, y_fakeB)
        # update discriminator for C -> [scanner1/scanner2]
        dC_loss1=d_model_C.train_on_batch(X_realA,y_realA)
        dC_loss12=d_model_C.train_on_batch(X_fakeA,y_realA)
        dC_loss2=d_model_C.train_on_batch(X_realB,y_fakeB)
        dC_loss22=d_model_C.train_on_batch(X_fakeB,y_fakeB)
        # summarize performance
        print('>%d, dA[%.3f,%.3f] dB[%.3f,%.3f] g[%.3f,%.3f] dC1[%.3f,%.3f] dC2[%.3f,%.3f] disclossBA[%.3f,%.3f]  identityAB[%.3f,%.3f]  cycle1BA[%.3f,%.3f] cycle1AB[%.3f,%.3f] ' % (i+1, dA_loss1,dA_loss2, dB_loss1,dB_loss2, g_loss1,g_loss2,dC_loss1,dC_loss12,dC_loss2,dC_loss22,disclossBA, disclossAB, identityAB,identityBA,cycle1BA,cycle2BA,cycle1AB,cycle2AB))
        ...
        dC1_hist.append(dC_loss1)
        dC12_hist.append(dC_loss12)
        dC2_hist.append(dC_loss2)
        dC22_hist.append(dC_loss22)
        
        # evaluate the model performance every so often
        plot_history(dA1_hist, dA2_hist, dB1_hist,dB2_hist,g1_hist,g2_hist,dC1_hist,dC12_hist,dC2_hist,dC22_hist)
        if (i+1) % (bat_per_epo * 1) == 0:
            # plot A->B translation
            summarize_performance(i, g_model_AtoB, trainA, trainB,'AtoB','BtoA')
            # plot B->A translation
        if (i+1) % (bat_per_epo * 5) == 0:
            # save the models
            save_models(i, g_model_AtoB, g_model_BtoA)
        if dC_loss12>0.3 or dC_loss22>0.3:
            summarize_performance(i, g_model_AtoB, trainA, trainB,'AtoB','BtoA')
            # plot B->A translation
            save_models(i, g_model_AtoB, g_model_BtoA)
...
d_model_C=define_discriminator(image_shape)
# composite: A -> B -> [real/fake, A]
c_model_AtoB = define_composite_model(g_model_AtoB, d_model_B,d_model_C, g_model_BtoA, image_shape)
# composite: B -> A -> [real/fake, B]
c_model_BtoA = define_composite_model(g_model_BtoA, d_model_A, d_model_C, g_model_AtoB, image_shape)
# train models
train(d_model_A, d_model_B, d_model_C,g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, dataset)
